...
- `SuperVibeBot.js`: RisuAI auto-update target and plain filename
- `SuperVibeBot.update.js`: same build, compatibility alias
...
- Updates CI and runtime diagnostics to validate the raw `refs/heads/main` update channel.

## 1.5.82

- Switches the plugin update URL to raw `refs/heads/main` to avoid stale `/main` raw cache during rapid releases.

## 1.5.81

- Routes image/asset target aliases through the asset executor instead of raw target comparisons.
- Recovers image-focused character-update actions that contain asset prompt payloads into asset create actions.
- Shows stop/cancel controls directly inside the visible Kero work-in-progress bubble.
- Keeps header, workstream, and in-progress stop/cancel buttons wired to one shared runtime interruption handler.
...
- Clears approval-required mission steps when proposal actions are approved or rejected, preventing mobile approval from leaving stale blocked actions behind.
- Reconciles orphaned approval-required steps when the proposal panel has no matching pending proposal after mobile/webview reloads.
- Treats explicit proposal approval as the confirmation for nested action prompts, including work-target delete confirmation paths.
...
- Narrows planning goal approval so casual replies such as "좋아" or "진행해" no longer switch into goal execution by themselves.
- Shows the current goal or pending goal conversion in the workstream panel.
- Adds stop/cancel controls that abort model, sub-agent, action, resume, and bulk auto-resume paths instead of only changing the UI.
- Adds clearer main-model and sub-agent activity events during long waits.

## 1.5.73

- Gives planning mode a read-only selected/reference context, so it can discuss checked bots without asking the user to paste their contents.
- Keeps planning mode non-mutating while preserving visibility into descriptions, first messages, lorebooks, regex, triggers, variables, and assets allowed by the reference scope.
...
- Points auto-update at raw `SuperVibeBot.update.js`, the compatibility file that returns fresh non-empty metadata in Risu-style `Range: bytes=0-512` checks.
...
- Moves auto-update to jsDelivr `@main/SuperVibeBot.js` to avoid GitHub raw branch/file cache lag.

## 1.5.57

- Points auto-update at the fresh `SuperVibeBot.js` raw branch file instead of the file-specific stale `SuperVibeBot.update.js` cache.
